...
但是此违例发生在刚做完CTS的clock path上，更改driver不仅会造成driver上一级的高负载（mos管换成更大的之后不仅会减小其本身的电阻，还会增大其本身的电容）还有可能对skew造成影响(driver->SRAM CK之间的latency降低，上一级->driver的latency升高，最终改变该路径的latency,进而对branch-to-branch skew造成影响）
...
考虑view的问题，可能是set_interactive_constraint_modes [all_constraint_modes -active]没有写好，导致它没在违例的view下生效，于是对view进行检查
report_analysis_views -type active
set p [get_pins proc0/cmem0/ddata0/u0/id0/CK]
foreach v [all_setup_analysis_views] {
puts " $v"
report_constraint -drv_violation_type max_transition \
  -view $v $p
}
然而发现每条每个view都约束的0.5
可能是因为普通的set_max_transition默认不能用一个更松的值覆盖已有的更严格DRV，则选择一条路径验证
set_global timing_constraint_enable_drv_limit_override true
set p [get_pins proc0/cmem0/ddata0/u0/id0/CK]
set_global timing_constraint_enable_drv_limit_override true
set_interactive_constraint_modes [all_constraint_modes]
set_max_transition 1.0 $p -override
set_interactive_constraint_modes {}
get_property $p max_transition \
    -view func_ssg_rcworst_CCworst_T_m40
report_constraint \
